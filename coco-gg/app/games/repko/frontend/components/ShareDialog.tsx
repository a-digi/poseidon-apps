  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        let url: string;
        if (baseUrl !== null) {
          url = `${baseUrl}/plugins/coco-gg/?mode=mobile&game=repko&t=${session.token}&room=${roomCode}`;
        } else {
          ...
          url = `http://${addresses[0]}:2014/plugins/coco-gg/?mode=mobile&game=repko&t=${session.token}&room=${roomCode}`;
        }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-slate-900">Invite players to Repko</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
