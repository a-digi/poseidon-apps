...
export function Board({
  ...
  const civColorById = useMemo(() => {
    const m = new Map<string, string>();
    const civs = state.civilizations ?? [];
    for (const c of civs) m.set(c.id, c.color);
    return m;
  }, [state.civilizations]);

  const civFlagById = useMemo(() => {
    const m = new Map<string, string>();
    const civs = state.civilizations ?? [];
    for (const c of civs) m.set(c.id, c.flag);
    return m;
  }, [state.civilizations]);

  const playerColorAndFlag = useMemo(() => {
    const m = new Map<string, { color: string; flag: string }>();
    for (const p of state.players) {
      const civColor = civColorById.get(p.civilizationId);
      const flag = civFlagById.get(p.civilizationId) ?? '';
      m.set(p.id, { color: civColor ?? p.color, flag });
    }
    return m;
  }, [state.players, civColorById, civFlagById]);
  ...
  const rendered = useMemo<RenderedTile[]>(() => {
    return tiles.map((tile) => {
      ...
      const strokeWidth = tile.ownerId !== '' ? 2.5 : 1.5;
  ...
  const yieldFont = Math.max(10, Math.round(12 * Math.min(zoom, 2)));
  const badgeFont = 9;
  const stackFont = 9;
        ...
        {rendered.map((r) => {
          ...
          return (
            ...
            >
              ...
              {showTier2 && productionIcon !== '' && (
                <text
                  x={r.cx - HEX_SIZE * 0.6}
                  y={r.cy - HEX_SIZE * 0.25}
                  textAnchor="middle"
                  style={{ fontSize: 10 }}
                >
                  {productionIcon}
