...
export function SharePhoneDialog({ onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        const url = `http://${addresses[0]}:2014/plugins/mbud/?t=${session.token}`;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-slate-900">Open on phone</h2>
