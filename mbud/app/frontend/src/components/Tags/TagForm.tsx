...
interface TagFormProps {
  initial: Tag | null;
  onSave: (payload: Omit<Tag, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>;
  onClose: () => void;
}
...
function init(t: Tag | null): FormState {
  return { name: t?.name ?? '' };
}

export function TagForm({ initial, onSave, onClose }: TagFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) { setNameInvalid(true); return; }
    setBusy(true);
    setError(null);
    try {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-slate-900">{initial ? 'Edit tag' : 'Add tag'}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">
            ...
            <input type="text" value={form.name} onChange={setName} className={nameInvalid ? inputErr : input} />
